...
interface YarnCountItem {
  id: string;
  countName: string;
  isActive: boolean;
}

const emptyForm = { countName: "", isActive: true };

export default function YarnCountsPage() {
  const [items, setItems] = useState<YarnCountItem[]>([]);
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editing, setEditing] = useState<YarnCountItem | null>(null);
  const [form, setForm] = useState(emptyForm);
  const [deleteOpen, setDeleteOpen] = useState(false);
  const [deleting, setDeleting] = useState<YarnCountItem | null>(null);

  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch("/api/yarn-counts");
      if (!res.ok) throw new Error();
      setItems(await res.json());
    } catch {
      toast.error("Failed to load yarn counts");
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  function openEdit(item: YarnCountItem) {
    setEditing(item);
    setForm({ countName: item.countName, isActive: item.isActive });
    setDialogOpen(true);
  }

  async function handleSave() {
    if (!form.countName.trim()) { toast.error("Count Name is required"); return; }
    setSaving(true);
    try {
      const url = editing ? `/api/yarn-counts/${editing.id}` : "/api/yarn-counts";
  ...
  async function handleDelete() {
    if (!deleting) return;
    try {
      const res = await fetch(`/api/yarn-counts/${deleting.id}`, { method: "DELETE" });
      if (!res.ok) throw new Error();
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-xl font-semibold tracking-tight">Yarn Counts</h1>
        </div>
        <Button size="sm" className="h-8 text-[13px]" onClick={openAdd}><Plus className="h-4 w-4 mr-1" /> Add Count</Button>
      </div>
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead className="text-[11px] uppercase tracking-wide">Count Name</TableHead>
            ...
            <TableBody>
              {loading ? (
                ...
                <TableRow><TableCell colSpan={4} className="text-center py-12 text-muted-foreground">No yarn counts found.</TableCell></TableRow>
              ) : items.map((item, idx) => (
                <TableRow key={item.id} className="cursor-pointer hover:bg-muted/50" onClick={() => openEdit(item)}>
                  ...
                  <TableCell className="text-[13px] font-medium">{item.countName}</TableCell>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader><DialogTitle>{editing ? "Edit Yarn Count" : "Add Yarn Count"}</DialogTitle></DialogHeader>
          <div className="space-y-4 py-4">
            <div className="space-y-1.5">
              <Label className="text-[13px]">Count Name <span className="text-destructive">*</span></Label>
              <Input value={form.countName} onChange={(e) => setForm({ ...form, countName: e.target.value })} placeholder="e.g. 30s, 40s, 2/60s" />
            </div>
            <div className="flex items-center gap-2">
      ...
      <Dialog open={deleteOpen} onOpenChange={setDeleteOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader><DialogTitle>Delete Yarn Count</DialogTitle></DialogHeader>
          <p className="text-sm text-muted-foreground py-4">Delete <strong>{deleting?.countName}</strong>?</p>
          <div className="flex justify-end gap-2">
